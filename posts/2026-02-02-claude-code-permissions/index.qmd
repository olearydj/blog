...
- `allow` grants read/write to the uv cache so sandboxed commands can access it (note the `//` prefix for absolute paths). Everything else is handled by the sandbox.
...
The sandbox blocks `.git` access broadly - not just the repository directory, but `.git` marker files anywhere. This prevents `git clone` into non-project directories and can affect tools whose caches contain `.git` marker files. Granting read/write access to specific cache paths via `allow` rules (as shown above) resolves this.
...
- [Claude Code's Instruction Mechanisms](/posts/2026-01-02-til-claude-code-instructions/) (CLAUDE.md, skills, rules, and hooks)

...
On first use in a session, `op` prompts for biometric auth (Touch ID on macOS). After that, secrets flow without interruption. Biometric unlock requires the 1Password desktop app with [app integration enabled](https://developer.1password.com/docs/cli/get-started/#turn-on-the-1password-desktop-app-integration).
